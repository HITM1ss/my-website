...
export const backgroundWallpaper: BackgroundWallpaperConfig = {
	...
	common: {
		...
		homeText: {
			...
			// 主页横幅主标题字体大小
			titleSize: "3.8rem",
			// 主页横幅副标题；API 暂不可用时显示此文案
			subtitle: "聚是一团火，散是满天星",
			// 网易云热评：仅在构建时请求一次，访客不会直接请求第三方 API
			// 将密钥设置为部署平台的 APIZERO_API_KEY 环境变量，切勿写入此文件
			neteaseComment: {
				enable: true,
				// 浏览器缓存 24 小时，避免重复获取同源静态结果
				cacheHours: 24,
				// 横幅空间有限，过长热评将被截断
				maxLength: 88,
			},
